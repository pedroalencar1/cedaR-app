...
ui <- fluidPage(
  ...
  of 29 stations in Europe. The selected stations provided by the ECA&D have over 100 years of consistent measurements
  very low gap occurrence in the data (< 0.3%). ',style="margin: 0;"),
  h5(align = "left", 
                    'Additional information on the data presented can be obtained in the paper
     "Which Droughts Are Becoming More Frequent? A Copula Entropy Analysis on the Recurrence Time of Droughts In Europe" 
     (Alencar, 2023 - Natural Hazards - under prep.).',
                    style="margin: 0;")
  )),
  br(),
...
server <- function(input, output, session) {
  ...
  observe(label = "map_desing",{
    ...
    output$Map1 <- renderLeaflet({
      ...
      data_selec <- data_stations[data_stations$station == id,]
  ...
  observe(label = "plot1", {
    ...
    my_image <- figures[grepl(paste0("_",id,"_op3"), figures)]
